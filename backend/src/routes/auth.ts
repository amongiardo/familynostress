...
async function registerLocalUser(input: {
  ...
  if (existing) {
    return { ok: false as const, status: 409, body: { error: 'User already exists' } };
  }
  ...
  if (input.inviteToken) {
    ...
    if (
      ...
      return { ok: false as const, status: 400, body: { error: 'Invite not valid for this email' } };
    }
  ...
  if (input.inviteToken) {
    ...
    if (!input.familyName || typeof input.familyName !== 'string' || !input.familyName.trim()) {
      return { ok: false as const, status: 400, body: { error: 'Missing family name' } };
    }
...
router.post('/local/register', async (req, res, next) => {
  try {
    ...
    const registration = await registerLocalUser({ email, password, name, inviteToken, familyName });
    if (!registration.ok) {
      return res.status(registration.status).json(registration.body);
    }
    const { user, activeFamilyId } = registration;
...
router.post('/local/register-token', async (req, res, next) => {
  try {
    const { email, password, name, inviteToken, familyName, tokenName, expiresInDays } = req.body ?? {};
    if (!email || !password || !name) {
      return res.status(400).json({ error: 'Missing email, password, or name' });
    }
    ...
    if (activeFamilyId) {
      await logAudit({
        ...
        userId: user.id,
        action: 'API_TOKEN_REGISTER_ISSUED',
